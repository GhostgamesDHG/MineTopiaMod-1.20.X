...
public class AtmdepositButtonMessage {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = AtmdepositMenu.guistate;
		// security measure to prevent arbitrary chunk generation
		if (!world.hasChunkAt(new BlockPos(x, y, z)))
			return;
		if (buttonID == 0) {

			AtmDeposit50Procedure.execute(world, entity);
		}
		if (buttonID == 1) {

			AtmDeposit5Procedure.execute(world, entity);
		}
		if (buttonID == 2) {

			AtmDeposit10Procedure.execute(world, entity);
		}
		if (buttonID == 3) {

			AtmDeposit20Procedure.execute(world, entity);
		}
		if (buttonID == 4) {

			AtmDeposit100Procedure.execute(world, entity);
		}
		if (buttonID == 5) {

			AtmDeposit1000Procedure.execute(world, entity);
		}
	}

	@SubscribeEvent
	public static void registerMessage(FMLCommonSetupEvent event) {
		MineTopiaMod.addNetworkMessage(AtmdepositButtonMessage.class, AtmdepositButtonMessage::buffer, AtmdepositButtonMessage::new, AtmdepositButtonMessage::handler);
	}
}
